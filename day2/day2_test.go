...
func init() {
	for _, v := range strings.Split(input, "\n") {
		keyVal := append([]string{}, strings.Split(v, ":")...)
		values = append(values, keyVal)
	}
}

func TestValidateCharsRepetitions(t *testing.T) {
	type args struct {
		values [][]string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotValids := day2.ValidateCharsRepetitions(tt.args.values); gotValids != tt.wantValids {
				t.Errorf("ValidateCharsRepetitions() = %v, want %v", gotValids, tt.wantValids)
			}
		})
	}
}

func TestValidateCharsLocations(t *testing.T) {
	type args struct {
		values [][]string
	}
	tests := []struct {
		...
		{
			name: "validating only one char locations match",
			args: args{
				values: values,
			},
			wantValids: 308,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotValids := day2.ValidateCharsLocations(tt.args.values); gotValids != tt.wantValids {
				t.Errorf("ValidateCharsLocations() = %v, want %v", gotValids, tt.wantValids)
			}
		})
	}
}
